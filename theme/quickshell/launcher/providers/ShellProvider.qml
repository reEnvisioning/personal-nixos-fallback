...
Item {
    ...
    Process {
        id: historyLoader
        command: ["sh", "-c", "cat $HOME/.config/reEnvisioning/appdata/quickshell/shell-history.json 2>/dev/null || echo '[]'"]
        running: true
    ...
    function save() {
        ...
        historySaver.command = ["sh", "-c",
            ...
            "cat > $HOME/.config/reEnvisioning/appdata/quickshell/shell-history.json << '" + delim + "'\n" +
            json + "\n" +
            delim]
