...
Item {
    ...
    function removeAt(index) {
        ...
        if (entry && entry.storagePath && entry.storagePath.length > 0) {
            Quickshell.execDetached(["sh", "-c",
                "rm -f $HOME/.config/reEnvisioning/appdata/quickshell/" + entry.storagePath])
        }
    ...
    function clearAll() {
        ...
        var stalePaths = root.entries
            ...
            .map(function(e) { return "\"$HOME/.config/reEnvisioning/appdata/quickshell/" + e.storagePath + "\"" })
    ...
    function copyAt(index) {
        ...
        if (entry.mimeType === "image/png" && entry.storagePath) {
            ...
            Quickshell.execDetached(["sh", "-c",
                "wl-copy --type image/png < \"$HOME/.config/reEnvisioning/appdata/quickshell/" + entry.storagePath + "\""])
        } else {
    ...
    function save() {
        ...
        saveProcess.command = ["sh", "-c",
            ...
            "cat > $HOME/.config/reEnvisioning/appdata/quickshell/clip-history.json << '" + delim + "'\n" +
            json + "\n" +
            delim]
    ...
    Process {
        id: loadProcess
        command: ["sh", "-c", "cat $HOME/.config/reEnvisioning/appdata/quickshell/clip-history.json 2>/dev/null || echo '[]'"]
        running: true
        stdout: StdioCollector {
            onStreamFinished: {
                try {
