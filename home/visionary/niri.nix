...
in {
  xdg.configFile."niri/config.kdl".text = ''
    ...
    spawn-sh-at-startup "status=$(cat /tmp/wg-vpn-status 2>/dev/null); if [ \"$status\" = \"connected\" ]; then notify-send -a Proxy VPN connected; elif [ \"$status\" = \"unreachable\" ]; then notify-send -a Proxy 'VPN server unreachable'; fi"
