...
in {
  xdg.configFile."niri/config.kdl".text = ''
    ...
    spawn-sh-at-startup "for i in $(seq 30); do status=$(cat /tmp/wg-vpn-status 2>/dev/null); case $status in connected) notify-send -a Proxy VPN connected && break;; unreachable) notify-send -a Proxy 'VPN server unreachable' && break;; esac; sleep 1; done"
