...
in {
  xdg.configFile."niri/config.kdl".text = ''
    ...
    spawn-sh-at-startup "status=$(cat /tmp/wg-vpn-status 2>/dev/null); case $status in connected) notify-send -a Proxy 'VPN connected';; unreachable) notify-send -a Proxy 'VPN server unreachable — using direct connection';; esac"
